...
u32  GB_ADDR(u8 *HZ,u8 HZ_type)//获得汉字点阵的地址
{
    ...
    else //chinese  for BG2312,GBK,GB18030,GB12345
    {
		...
		if( HZ_type == TYPE_12 )
			temp = (temp+846)*24 + ZF1212Y_ADDR;
		else if( HZ_type == TYPE_16 )
			temp = (temp+846))*32 + ZF1616Y_ADDR;
			////GT23H24T3Y
			//temp = (temp+846-(94+90))*32 + ZF1616Y_ADDR;
		else if( HZ_type == TYPE_24 )
			temp = (temp+846)*72 + ZF2424Y_ADDR;
		else if( HZ_type == TYPE_32 )
			temp=(temp+846)*128+ZF3232Y_ADDR;
		else
			return(0x00);
	}
	return temp;
...
/*u32  GB_ADDR(u8 *HZ,u8 HZ_type)//获得汉字点阵的地址
{
    ...
    else //chinese  for BG2312,GBK,GB18030,GB12345
    {
        	...
        	if( (c1>=0xA1 && c1 <= 0xA3 && c2>=0xa1))
        	{
				...
				{
					address=((c1-0xA1)*94+(c2-0xA1))*32+ZF1616Y_ADDR;
				}
				else if( HZ_type == TYPE_24 )
				{
					return( temp *72 + ZF2424Y_ADDR);
			...
			else if(c1==0xA6&&c2>=0xa1){
				if( HZ_type == TYPE_16 )
				{
					address=((c1-0xA1)*94+(c2-0xA1)-94*2)*32+ZF1616Y_ADDR;
				}
			}
			else if(c1==0xA9&&c2>=0xa1){
				if( HZ_type == TYPE_16 )
				{
					address=((c1-0xA1)*94+(c2-0xA1)-94*4)*32+ZF1616Y_ADDR;
				}
			}
				...
				if( HZ_type == TYPE_16 )
				{
					address=((c1-0xA8)*96+(c2-0x40)+94*5)*32+ZF1616Y_ADDR;
				}
			}
			else if(c1>=0xB0&&c1<=0xF7&&c2>=0xa1){
				if( HZ_type == TYPE_16 )
				{
					address=((c1-0xB0)*94+(c2-0xa1)+662)*32+ZF1616Y_ADDR;
				}
			}
