...
unsigned char localAddress,unitsNum, curSentenceIndex,changeMessageTimer=0,currentUnitIndex=1,currentUnitTotalIndex,commState1,commState2,reqUnitInfoInteval,oldCommState=COMM_STATE_NOTHING,staffBufIndex=0,oldTH0=0,oldTL0=0,timeoutThreshold=0,oldGroup=255,retryCount=0;
...
uchar xdata stateStat[4]={0};//索引0未用 ，1 表示矿灯取走 2表示矿灯充电 3表示矿灯充满
...
void switchIfNecessary(uchar lampNo){
	uchar group=(lampNo-1)/10;//每十个一组 cd4067
//	if(group==oldGroup){
//		return;
//	}
	oldGroup=group;
...
void sendLedMessages(){
	...
	IE2 = 0x00;
	ES=0; 				
	switchIfNecessary(101);//文字广播在第10组
...
void Timer0_isr(void) interrupt 1 using 1
{
	...
	{	
		reqStateTimer++;
		if(reqStateTimer>=20*queryStateInteval){	
			reqStateTimer=0;
			readStateFlag=1;
			currentUnitIndex=1;
		}
...
void refreshUnitsInfo(){
	uchar m,val;
	stateStat[0]=stateStat[1]=stateStat[2]=0;
	for(m=1;m<=unitsNum;m++){
