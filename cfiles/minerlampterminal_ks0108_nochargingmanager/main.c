...
uchar* xdata batStateDesc[]={"检测状态中","矿灯被取走","矿灯充电中","矿灯已充满","发生故障！"};
...
bit isSelected=0,updateStaffFlag=0,stateAdFlag=0,brightAdFlag=0,inSetting=0,inAddrSetting=0,isInfraredAllowed=0,isdebug=1;
...
void displayADC(){
		...
		//	LCD_PutString(0,4,"模拟量：");
  			LCD_PutSingleByteString(12,4,stateValueStr);
		...
		if(brightAdFlag==1){
			...
			adcBrightStr[3]='\0';
			LCD_PutSingleByteString(13,3,adcBrightStr);
		}
