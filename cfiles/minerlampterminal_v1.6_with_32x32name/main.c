...
uchar* xdata menu[MENU_LEN]={"地址设置","亮度设置","充满阈值","调试设置"};
...
uchar tready,rready,batState=0,brightValue=0,commState=0,bufIndex=0,updateStaffInteval,localAddress=0,curSentenceIndex,currentChannel=0,chargingFullThreshold=10,brightThreshold,isSetted,isInfoSetted=0,batAdValArrIndex=0;
unsigned int stateValue=0,msCount=0,mCount=0;
bit infoChanged=1,isSelected=0,updateStaffFlag=0,stateAdFlag=0,brightAdFlag=0,inSetting=0,inAddrSetting=0,isInfraredAllowed=0,isdebug=0,timeChanged=0,startSetting=0;
...
void storageBaseInfo(){
	...
	IapProgramByte(IAP_ADDRESS+VOLTAGE_THRESHOLD_ADDR, chargingFullThreshold); 
	for (m = 0; m < FIELD_COUNT; m++) {
...
#define MENU_STATE_IN_VOLTAGE_THRESHOLD_SETTING 7
#define MENU_STATE_IN_VOLTAGE_THRESHOLD_BIT_SETTING 8
#define MENU_STATE_IN_VOLTAGE_THRESHOLD_SETTING_FINISHED 9
uchar menuState=MENU_STATE_NOP;
uchar selAddrBit=0;
uchar selBrightThresholdBit=0;
uchar selVoltageThresholdBit=0;
uchar menuIndex=0;

uchar tempAddressArr[4]={'\0'};
uchar tempBrightThresholdArr[4]={'\0'};
uchar tempVoltageThresholdArr[4]={'\0'};
...
void initTempVoltageThresholdArr(){


	tempVoltageThresholdArr[0]='0'+(chargingFullThreshold/100);

	tempVoltageThresholdArr[1]='0'+((chargingFullThreshold%100)/10);

	tempVoltageThresholdArr[2]='0'+(chargingFullThreshold%10);
	tempVoltageThresholdArr[3]='\0';
}
...
void menuItemSeleted(uchar index){
	...
	case 0: //设置地址
		...
		initTempAddressArr();
		LCD_PutString(3,2,tempAddressArr);
	 ...
	 case 1: //亮度设置
		...
		LCD_PutString(3,2,tempBrightThresholdArr);
		LCD_PutChar(2,3,26);//输出"->"
		LCD_PutPosition(3+selBrightThresholdBit,3);
		
		break;
	 case 2: //充满阈值设置
		menuState=MENU_STATE_IN_VOLTAGE_THRESHOLD_SETTING;
		selVoltageThresholdBit=0;
		showMenuItem(menuIndex,0);
		initTempVoltageThresholdArr();
		LCD_PutString(3,2,tempVoltageThresholdArr);
		LCD_PutChar(2,3,26);//输出"->"
		LCD_PutPosition(3+selVoltageThresholdBit,3);
...
/*	if(k1==0)
	{
		...
		{	
			...
			if(inSetting==0){
				...
				else if(menuState==MENU_STATE_IN_VOLTAGE_THRESHOLD_BIT_SETTING){
					menuState=MENU_STATE_IN_VOLTAGE_THRESHOLD_SETTING;
				}
				else if(menuState==MENU_STATE_IN_VOLTAGE_THRESHOLD_SETTING){
					menuState=MENU_STATE_NOP;
				...
				else if(menuState==MENU_STATE_IN_VOLTAGE_THRESHOLD_SETTING_FINISHED){
	...
	if(k3==0)
	{
		...
		if(k3==0)
		{
			...
			if(menuState==MENU_STATE_NOP){
				...
				menuState=MENU_STATE_IN_VOLTAGE_THRESHOLD_BIT_SETTING;
			}
			else if(menuState==MENU_STATE_IN_VOLTAGE_THRESHOLD_BIT_SETTING){
				chargingFullThreshold=(tempVoltageThresholdArr[1]-'0')*100+(tempVoltageThresholdArr[3]-'0')*10+(tempVoltageThresholdArr[5]-'0');
				storageBaseInfo();
				//关闭游标及反白
				LCD_PutString(2,3,"设置完毕!");
				menuState=MENU_STATE_IN_VOLTAGE_THRESHOLD_SETTING_FINISHED;
		...
		if(k5==0)
		{
			...
			if(inSetting){
				if(menuState==MENU_STATE_NOP){
					...
					selVoltageThresholdBit=(selVoltageThresholdBit-1+3)%3;
					LCD_PutPosition(3+selVoltageThresholdBit,3);
					
				}
				else if(menuState==MENU_STATE_IN_VOLTAGE_THRESHOLD_BIT_SETTING){
					tempVoltageThresholdArr[2*selVoltageThresholdBit+1]='0'+((tempVoltageThresholdArr[2*selVoltageThresholdBit+1]-'0'-1+10)%10);
					LCD_PutString(3,2,tempVoltageThresholdArr);;
					LCD_PutPosition(3+selVoltageThresholdBit,3);
		...
		if(k7==0)
		{
			...
			if(inSetting){
				if(menuState==MENU_STATE_NOP)
				{
					...
					selVoltageThresholdBit=(selVoltageThresholdBit+1)%3;
					LCD_PutPosition(3+selVoltageThresholdBit,3);
					
				}
				else if(menuState==MENU_STATE_IN_BRIGHT_THRESHOLD_BIT_SETTING){
					tempVoltageThresholdArr[2*selVoltageThresholdBit+1]='0'+((tempVoltageThresholdArr[2*selVoltageThresholdBit+1]-'0'+1)%10);
					LCD_PutString(3,2,tempVoltageThresholdArr);
					LCD_PutPosition(3+selVoltageThresholdBit,3);
...
void retrieveBaseInfo(){
	...
	chargingFullThreshold=IapReadByte(IAP_ADDRESS+VOLTAGE_THRESHOLD_ADDR);
	if(chargingFullThreshold>=0x50){
		chargingFullThreshold=10;
	}
	for (m = 0; m < FIELD_COUNT; m++) {
		for (n = 0; n < STAFF_INFO_FIELD_LENGTN; n++) {
...
void stateTest(){
	...
	for(m=0;m<BAT_VAL_ARR_LEN;m++){
		...
		if(zeroCount>7||everage<=chargingFullThreshold){
				if(emptyCount<250){
					emptyCount++;
				}
				fullCount=0;
		...
		if(fullRangeCount>4 ||everage>chargingFullThreshold){
			emptyCount=0;
			if(fullCount<250){
				fullCount++;
			}
...
void Ir_work(void);
void Ircordpro(void);
//一个按键编码被红外接收头接收后，会触发多次外部终端，根据每次的触发时间长短（衡量标准为irtime的大小），来决定每个按键的32位编码。irdata数组
//中存储的是每一位的延续时间，延续时间大于7，则认为该位对应的二进制值是1，否则为0
...
{	
	...
	case IR_PREV:
	{
		...
		else if(menuState==MENU_STATE_IN_VOLTAGE_THRESHOLD_SETTING||menuState==MENU_STATE_IN_VOLTAGE_THRESHOLD_BIT_SETTING){
			selVoltageThresholdBit=(selVoltageThresholdBit-1+3)%3;
			LCD_PutPosition(3+selVoltageThresholdBit,3);			
		}
	...
	case IR_VOL_MINUS:
	{
		if(menuState==MENU_STATE_NOP)
		{
			...
			showMenuItem(menuIndex,1);
				...
				tempVoltageThresholdArr[selVoltageThresholdBit]='0'+((tempVoltageThresholdArr[selVoltageThresholdBit]-'0'-1+10)%10);
				LCD_PutString(3,2,tempVoltageThresholdArr);
				LCD_PutPosition(3+selVoltageThresholdBit,3);
	...
	case IR_VOL_ADD:
	{
		if(menuState==MENU_STATE_NOP)
		{
			...
			showMenuItem(menuIndex,1);
				...
				tempVoltageThresholdArr[selVoltageThresholdBit]='0'+((tempVoltageThresholdArr[selVoltageThresholdBit]-'0'+1)%10);
				LCD_PutString(3,2,tempVoltageThresholdArr);
				LCD_PutPosition(3+selVoltageThresholdBit,3);
	...
	case IR_EQ:
	{
		if(menuState==MENU_STATE_NOP){
			...
			chargingFullThreshold=(tempVoltageThresholdArr[0]-'0')*100+(tempVoltageThresholdArr[1]-'0')*10+(tempVoltageThresholdArr[2]-'0');
			storageBaseInfo();
			//关闭游标及反白
			LCD_PutString(2,3,"设置完毕!");
			menuState=MENU_STATE_IN_VOLTAGE_THRESHOLD_SETTING_FINISHED;		
	...
	case IR_NINE:
	{
		if(menuState==MENU_STATE_IN_ADDR_SETTING||menuState==MENU_STATE_IN_ADDR_BIT_SETTING){
			...
			tempVoltageThresholdArr[selVoltageThresholdBit]='0'+changeNum(IRcord[2]);
			LCD_PutString(3,2,tempVoltageThresholdArr);;
			LCD_PutPosition(3+selVoltageThresholdBit,3);
