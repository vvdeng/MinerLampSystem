...
uchar* code fieldName[]={"姓名：","部门：","工种："};
uchar* code menu[]={"地址设置","亮度设置","红外设置"};
uchar xdata staffInfoBuf[FIELD_COUNT][STAFF_INFO_FIELD_LENGTN+1]={'\0'};
uchar xdata buf[2*FIELD_COUNT*STAFF_INFO_FIELD_LENGTN];

uchar tready,rready,stateValue=0,brightValue=0,commState=0,bufIndex=0,updateStaffInteval,localAddress=0,curSentenceIndex,currentChannel=0,brightThreshold;
...
void displayADC(){
		...
		if(stateAdFlag==1){
			...
			simuStateValue=((stateValue%100)/25);
